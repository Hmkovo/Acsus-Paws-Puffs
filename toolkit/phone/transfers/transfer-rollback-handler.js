...
export function initTransferRollbackHandler() {
    registerRollbackHandler({
        ...
        rollback: async (contactId, deletedMessages, deletedMessageIds) => {
            logger.debug('phone','[TransferRollback] 开始回退转账消息');
            ...
            for (const msgId of deletedMessageIds) {
                ...
                if (transaction) {
                    logger.debug('phone','[TransferRollback] 发现需要删除的转账记录:', {
                    ...
                    if (success) {
                        ...
                        logger.warn('phone','[TransferRollback] 删除转账记录失败:', transaction.id);
                    }
                }
            }

            if (deletedCount > 0) {
                logger.info('phone','[TransferRollback] 共回退', deletedCount, '条转账记录');
                logger.debug('phone','[TransferRollback] 删除的转账:', deletedTransfers.map(t => 
                ...
                try {
                    ...
                    if (walletPage && walletPage.closest('.phone-popup')) {
                        logger.debug('phone','[TransferRollback] 检测到钱包页面打开，刷新UI');
                        // 触发钱包数据变化事件，让钱包页自动刷新
                    ...
                    logger.warn('phone','[TransferRollback] UI刷新失败（不影响数据回退）:', uiError);
                }
            } else {
                logger.debug('phone','[TransferRollback] 没有需要回退的转账记录');
            }
        }
    });

    logger.info('phone','[TransferRollback] 转账消息回退处理器已初始化');
}

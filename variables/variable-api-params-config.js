/**
 * 变量模块 - AI API参数配置（基于酒馆的data-source规则）
 * @module variables/variable-api-params-config
 *
 * @description
...
export function getSupportedParams(format) {
  ...
  logger.debug('[VariableAPIParams.getSupportedParams] 格式:', format, '→ 支持参数:', params);
  return params;
}
...
export function getDefaultParams(format) {
  ...
  logger.debug('[VariableAPIParams.getDefaultParams] 格式:', format, '→ 默认值:', defaults);
  return defaults;
}
...
export function validateParamValue(paramName, value) {
  const definition = PARAMS_DEFINITIONS[paramName];
  if (!definition) {
    logger.warn('[VariableAPIParams.validateParamValue] 未知参数:', paramName);
    return false;
  }
